...
//std::optional<OpenFaceExtractor::Output> OpenFaceExtractor::operator()(const std::string& filePath, unsigned long size, const Alignment alignment)
std::optional<OpenFaceExtractor::Output> OpenFaceExtractor::operator()(const std::string& filePath, unsigned long size)
{
    ...
    constexpr const unsigned long(&lkIds)[3] = outerEyesAndNose;
    //const unsigned long(&lkIds)[3] = (alignment == Alignment::InnerEyesAndBottomLip ? innerEyesAndBottomLip : outerEyesAndNose);

    // Extract the coordinates of the landmarks we are interested in
    //std::vector<cv::Point2f> inPts{ std::size(lkIds) };
    ...
    constexpr auto xMinMax = std::minmax_element(std::cbegin(lkTemplate), std::cend(lkTemplate), [](auto a, auto b) { return a[0] < b[0]; });
    constexpr auto yMinMax = std::minmax_element(std::cbegin(lkTemplate), std::cend(lkTemplate), [](auto a, auto b) { return a[1] < b[1]; });

    //const auto [itMinX, itMaxX] = std::minmax_element(std::begin(lkTemplate), std::end(lkTemplate), [](auto a, auto b) { return a[0] < b[0]; });
    //const auto [itMinY, itMaxY] = std::minmax_element(std::begin(lkTemplate), std::end(lkTemplate), [](auto a, auto b) { return a[1] < b[1]; });
    ...
    std::array<cv::Point2f, std::size(lkIds)> outPts;
    std::transform(std::begin(lkIds), std::end(lkIds), outPts.begin(), 
        [size, minX = (*xMinMax.first)[0], maxX = (*xMinMax.second)[0], minY = (*yMinMax.first)[1], maxY = (*yMinMax.second)[1]](auto lkId)
        {
    ...
    //cv::warpAffine(im, alignedFace, t, cv::Size(size, size), cv::INTER_CUBIC, cv::BORDER_REFLECT);
